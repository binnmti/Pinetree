...
function initializeTemporarySave(dotNetHelper) {
    try {
...
export function saveTemporary(id, title, content, originalTitle, originalContent) {
    try {
...
export function loadTemporary(id) {
    try {
        const key = `pinetree_temp_save_${id}`;
...
export function deleteTemporarySave(id) {
    try {
        const key = `pinetree_temp_save_${id}`;
        localStorage.removeItem(key);
...
export function hasTemporarySave(id) {
    try {
        const key = `pinetree_temp_save_${id}`;
...
export function cleanupOldTemporarySaves(maxAgeHours = 24) {
    try {
        const now = Date.now();
...
export function getTemporarySaveStatus(id) {
    try {
        const key = `pinetree_temp_save_${id}`;
        const data = localStorage.getItem(key);
        if (!data) {
...
export function setupScrollSync(textArea, previewContainer) {
    ...
    function getScrollPosition(textarea) {
        ...
        for (let i = 0; i < lines.length && elementIndex < previewElements.length; i++) {
                ...
                if (!inCodeBlock) {
                    ...
                    if (elementIndex < previewElements.length) {
                        for (let j = codeBlockStartLine; j <= i; j++) {
                            lineToElementMap.set(j, previewElements[elementIndex]);
                        }
                        elementIndex++;
                    }
                }
            ...
            if (trimmedLine.includes('|') && trimmedLine.split('|').length > 2) {
                if (!inTable) {
                    inTable = true;
                    if (elementIndex < previewElements.length) {
                        lineToElementMap.set(i, previewElements[elementIndex]);
                        elementIndex++;
                    }
                }
                continue;
    ...
    function scrollPreviewToElement(element, scrollPos) {
        if (!element)
            return;
...
function setupResizer(resizer, targetPanel, minPercent, maxPercent) {
    ...
    function onMouseDown(e) {
        try {
            ...
            state.parentWidth = targetPanel.parentNode.getBoundingClientRect().width;
