...
let draggedItemTitle: string | null = null; // アイテムのタイトルも保存
...
function determineDropPosition(e: DragEvent | MouseEvent, item: HTMLElement): DropPosition {
    ...
    if (relativeY < 0.15) {
        if (DEBUG) console.log(`判定: ${DROP_POSITION.BEFORE} (上部15%)`);
        return DROP_POSITION.BEFORE;
    } else if (relativeY > 0.85) {
        if (DEBUG) console.log(`判定: ${DROP_POSITION.AFTER} (下部15%)`);
        return DROP_POSITION.AFTER;
    } else {
        if (DEBUG) console.log(`判定: ${DROP_POSITION.INTO} (中央70%)`);
