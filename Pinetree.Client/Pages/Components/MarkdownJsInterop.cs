...
public class MarkdownJsInterop(IJSRuntime jsRuntime) : IAsyncDisposable
{
    ...
    public async ValueTask SetupMarkdownClickListenerAsync<T>(ElementReference container, DotNetObjectReference<T> dotNetRef) where T : class
    {
        var module = await _moduleTask.Value;
    ...
    public async ValueTask InitResizersAsync()
    {
        var module = await _moduleTask.Value;
        await module.InvokeVoidAsync("initResizers");
    }
